...
Boost your digital research skills, accelerate your research impact—discover 80+ learning resources, events, and expert-led training in one central hub with the UKRI Digital Research Skills Catalyst. Find out more [here](./about.qmd).
...
:::{.primary-search}
```{ojs}
// This cell contains the main search bar.

// NB: The "show filters" button is programatically added later, so it does not appear here.
...
```{ojs}
//| output: false
// This cell implements the "search all" function by resetting all filters and inserting "*" into the search query.

// This is done by first updating the search_terms value to "*" and then simulating a click on the "reset_filters" button.
{
  search_all; // We need this line to trigger on an update to the "show all" button.
  if (search_all > 0) {
...
::: {.search-filters}
```{ojs}
// This cell implements the "show filters" button.
...
![](./assets/ukri-logo.png){fig-align="center" height="100px" fig-alt="Funded by the UKRI"}
