...
course_data.forEach(row => {
  ...
  // IDs and URLs
  jsonld["@id"] = row.identifier || row.url;
  jsonld["url"] = row.url;

  // Name & description
  jsonld["name"] = row.headline;
  jsonld["description"] = row.description;
  ...
  // Keywords – use exact set if provided
  if (row.keywords) {
    jsonld["keywords"] = row.keywords.split(",").map(k => k.trim());
  }

  // Authors – do NOT split commas inside names; treat as single strings
  if (row.author) {
    const authors = row.author.split(/;\s*/); // use ; to separate multiple authors if present
  ...
  // Language & free
  jsonld["inLanguage"] = row.inLanguage ? row.inLanguage.toLowerCase() : "en";
  jsonld["isAccessibleForFree"] = row.isAccessibleForFree === "yes";

  // Explicitly mark no course instance
